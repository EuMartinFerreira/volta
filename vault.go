...
// persistSecretsContainer encrypts and saves the unified secrets container to storage
func (v *Vault) persistSecretsContainer(container *SecretsContainer) error {
	// Validate container
	if container == nil {
		return fmt.Errorf("secrets container cannot be nil")
	}
	...
	containerData, err := json.Marshal(container)
	if err != nil {
		return fmt.Errorf("failed to marshal secrets container: %w", err)
	}

	// Encrypt the serialized container using current key
	encryptedData, err := v.encryptWithCurrentKey(containerData)
	if err != nil {
	...
	if err := v.store.SaveSecretsData(encryptedData); err != nil {
		return fmt.Errorf("failed to save secrets container to store: %w", err)
	}
...
func (v *Vault) loadSecretsContainer() error {
	// Load encrypted data from storage
	encryptedData, err := v.store.LoadSecretsData()
	if err != nil {
...
func (v *Vault) initializeSecretsContainer() error {
	...
	existingData, err := v.store.LoadSecretsData()
	if err != nil {
		if os.IsNotExist(err) {
	...
	debug.Print("initializeSecretsContainer: Loaded existing secrets data, size: %d bytes\n", len(existingData))

	// Store the ENCRYPTED data in enclave
	v.secretsContainer = memguard.NewEnclave(existingData)
	...
	debug.Print("initializeSecretsContainer: Successfully validated, found %d secrets\n", len(container.Secrets))
...
func (v *Vault) loadSecretsFromStore() error {
	...
	containerData, _, err := v.decryptWithCurrentKey(encryptedData)
	if err != nil {
		return fmt.Errorf("failed to decrypt secrets container: %w", err)
	}
