...
func (v *Vault) initializeKeys() error {
	...
	encryptedMetadata, err := v.store.LoadMetadata()
	if err != nil {
		debug.Print("initializeKeys: Failed to load metadata: %v\n", err)
		return fmt.Errorf("failed to load metadata: %w", err)
	}

	debug.Print("initializeKeys: Loaded metadata, size: %d bytes\n", len(encryptedMetadata))
...
func (v *Vault) loadOrCreateSalt(providedSalt []byte) error {
	...
	if exists {
		// Load existing salt
		existingSaltData, err := v.store.LoadSalt()
		if err != nil {
		...
		// Save to storage
		if err = v.store.SaveSalt(saltData); err != nil {
...
// isWeakKey performs cryptographic strength validation on key material to detect
// keys that are vulnerable to attack or unsuitable for secure encryption operations.
//
// This function implements multiple heuristic checks to identify keys that exhibit
// patterns or characteristics that significantly reduce their cryptographic security.
// It serves as a critical security control to prevent the use of predictable,
// low-entropy, or algorithmically weak key material in vault operations.
//
// VALIDATION CRITERIA:
//
//  1. LENGTH REQUIREMENT:
//     Keys must be at least 256 bits (32 bytes) to meet modern cryptographic
//     standards. Shorter keys are automatically rejected regardless of content
//     as they provide insufficient security against brute force attacks.
//
//  2. ALL-ZERO DETECTION:
//     Keys consisting entirely of zero bytes are rejected as they represent
//     uninitialized memory, default values, or intentionally weak test keys.
//     Such keys provide no cryptographic security and may indicate system
//     failures in key generation processes.
//
//  3. REPEATED BYTE PATTERN:
//     Keys where all bytes have the same value (e.g., all 0xFF, all 0xAA) are
//     rejected as they indicate algorithmic failures in random number generation
//     or deliberate selection of weak keys. These patterns are easily guessable
//     and provide minimal entropy.
//
//  4. ENTROPY ESTIMATION:
//     Keys must contain at least 16 distinct byte values across their length.
//     This heuristic check helps identify keys with insufficient randomness,
//     such as those generated from weak PRNGs, human-selected passwords encoded
//     as keys, or keys derived from limited input spaces.
//
// SECURITY RATIONALE:
// Weak keys can catastrophically compromise the security of encrypted data:
// • Predictable keys enable brute force attacks with reduced search spaces
// • Low-entropy keys may be vulnerable to dictionary or rainbow table attacks
// • Pattern-based keys can be reconstructed through cryptanalysis
// • Repeated or constant keys eliminate confidentiality protection entirely
//
// LIMITATIONS AND FALSE POSITIVES:
// This function uses heuristic analysis and may occasionally flag secure keys:
// • High-quality random keys might rarely have fewer than 16 unique bytes
// • Keys derived through legitimate KDF processes might exhibit some patterns
// • The entropy check is statistical and not cryptographically rigorous
//
// However, the security benefit of rejecting potentially weak keys outweighs
// the minimal risk of false positives, especially since key regeneration
// typically resolves any incorrectly flagged keys.
//
// CRYPTOGRAPHIC CONTEXT:
// Modern symmetric encryption algorithms (AES, ChaCha20) and key derivation
// functions (PBKDF2, scrypt, Argon2) assume input key material has sufficient
// entropy. Using weak keys can:
// • Reduce effective security to well below the intended level
// • Make encrypted data vulnerable to offline attacks
// • Compromise forward secrecy and other security properties
// • Enable practical attacks against otherwise secure implementations
//
// INTEGRATION WITH KEY GENERATION:
// This function should be called on all key material before use:
// • Keys generated from random sources (crypto/rand)
// • Keys derived from passwords or other user input
// • Keys loaded from external sources or configuration
// • Keys received over network connections or APIs
//
// PERFORMANCE CHARACTERISTICS:
// The function performs O(n) analysis where n is the key length, making it
// suitable for use in key validation paths. The entropy check uses a map
// with at most 256 entries, providing consistent performance regardless
// of key size.
//
// Parameters:
//   - key: The key material to validate, typically 32+ bytes of binary data
//
// Returns:
//   - bool: true if the key exhibits weak characteristics and should be rejected,
//     false if the key passes all heuristic security checks
//
// Example Usage:
//
//	// Validate generated key before use
//	key := make([]byte, 32)
//	if _, err := rand.Read(key); err != nil {
//	    return err
//	}
//	if isWeakKey(key) {
//	    return errors.New("generated key failed security validation")
//	}
//
//	// Validate derived key from user input
//	derivedKey := pbkdf2.Key(password, salt, 100000, 32, sha256.New)
//	if isWeakKey(derivedKey) {
//	    return errors.New("derived key is cryptographically weak")
//	}
//
//	// Key loading with validation
//	func (v *Vault) LoadKey(keyData []byte) error {
//	    if isWeakKey(keyData) {
//	        return fmt.Errorf("refusing to load weak key material")
//	    }
//	    // Proceed with key installation
//	}
