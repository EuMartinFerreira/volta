...
// reencryptAllSecretsUnified re-encrypts all secrets using the unified container approach
func (v *Vault) reencryptAllSecretsUnified(oldKeyID string) (int, error) {
	// Get the current secrets container
	container, err := v.getSecretsContainer()
	if err != nil {
		return 0, fmt.Errorf("failed to get secrets container: %w", err)
	}

	// If no secrets, nothing to re-encrypt
	...
	// Process each secret in the container
	for secretID, secretEntry := range container.Secrets {
		// Skip if this secret doesn't use the old key
		if secretEntry.Metadata.KeyID != oldKeyID {
			continue
		}

		// Temporarily switch to old key for decryption
		originalKeyID := v.currentKeyID
		v.currentKeyID = oldKeyID

		// Decrypt the secret data with the old key
		decryptedData, _, err := v.decryptWithCurrentKey(secretEntry.Data)
		if err != nil {
			v.currentKeyID = originalKeyID // Restore current key
			failures = append(failures, fmt.Sprintf("secret %s: failed to decrypt: %v", secretID, err))
			continue
		}

		// Switch back to new key for encryption
		v.currentKeyID = originalKeyID

		// Encrypt with the new key
		newEncryptedData, err := v.encryptWithCurrentKey(decryptedData)
		if err != nil {
			// Clear the decrypted data before continuing
			for i := range decryptedData {
				decryptedData[i] = 0
			}
			failures = append(failures, fmt.Sprintf("secret %s: failed to encrypt with new key: %v", secretID, err))
			continue
		}

		// Clear the decrypted data immediately after use
		for i := range decryptedData {
			decryptedData[i] = 0
		}

		// Update the secret entry with new encrypted data and metadata
		secretEntry.Data = newEncryptedData
		secretEntry.UpdatedAt = time.Now().UTC()
		secretEntry.Version++

		// Update the metadata with new key ID
		secretEntry.Metadata.KeyID = v.currentKeyID
		secretEntry.Metadata.UpdatedAt = time.Now().UTC()
		secretEntry.Metadata.Size = len(decryptedData)

		// Update the container entry
		container.Secrets[secretID] = secretEntry

		reencryptedCount++

		// Log successful re-encryption
		v.audit.Log("SECRET_REENCRYPTED", true, map[string]interface{}{
			"secret_id":   secretID,
			"old_key_id":  oldKeyID,
			"new_key_id":  v.currentKeyID,
			"new_version": secretEntry.Version,
		})
	}

	// If there were any failures, log them and return an error
	if len(failures) > 0 {
		errorMsg := fmt.Sprintf("failed to re-encrypt %d secrets: %v", len(failures), failures)
		v.audit.Log("REENCRYPT_PARTIAL_FAILURE", false, map[string]interface{}{
			"successful_count": reencryptedCount,
			"failed_count":     len(failures),
			"failures":         failures,
		})
		return reencryptedCount, fmt.Errorf(errorMsg)
	}

	// Save the updated container with re-encrypted secrets
	if reencryptedCount > 0 {
		if err = v.updateSecretsContainer(container); err != nil {
			return reencryptedCount, fmt.Errorf("failed to save updated secrets container: %w", err)
		}

		if err = v.persistSecretsContainer(container); err != nil {
			return reencryptedCount, fmt.Errorf("failed to persist updated secrets container: %w", err)
		}
	}
