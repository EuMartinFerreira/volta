...
func (m *mockFailingStore) SaveMetadata(metadata []byte) error {
	if m.failOnMetadata {
		return fmt.Errorf("mock store failure on metadata save")
	}
	return m.Store.SaveMetadata(metadata)
}
...
func (m *mockFailingStore) SaveSalt(salt []byte) error {
	if m.failOnSalt {
		return fmt.Errorf("mock store failure on salt save")
	}
	return m.Store.SaveSalt(salt)
}

func (m *mockFailingStore) SaveKeyMetadata(metadata []byte) error {
	if m.failOnMetadata {
		return fmt.Errorf("mock store failure on metadata save")
	}
	return m.Store.SaveMetadata(metadata)
