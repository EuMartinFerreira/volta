...
func (v *Vault) Restore(backupDir, passphrase string) error {
	...
	// ✅ Load the restored salt from storage
	debug.Print("Loading restored salt from storage")
	restoredSalt, err := v.store.LoadSalt()
	if err != nil {
		return fmt.Errorf("failed to load restored salt: %w", err)
	}
	v.derivationSaltEnclave = memguard.NewEnclave(restoredSalt)
	debug.Print("Loaded salt, first 16 bytes: %x", restoredSalt[:16])
...
func (v *Vault) collectBackupData() (*persist.BackupData, error) {
	...
	// Get salt data
	if salt, err := v.store.LoadSalt(); err != nil {
		if !misc.IsNotFoundError(err) {
			return nil, fmt.Errorf("failed to load salt: %w", err)
		}
	} else {
		backupData.Salt = salt
	}

	// Load vault metadata (keys + key metadata)
	if vaultMetadata, err := v.store.LoadMetadata(); err != nil {
		if !misc.IsNotFoundError(err) {
			return nil, fmt.Errorf("failed to load vault metadata: %w", err)
		}
	} else {
		backupData.VaultMetadata = vaultMetadata
	}
...
func (v *Vault) restoreBackupData(backupData *persist.BackupData) error {
	...
	if len(backupData.Salt) > 0 {
		if err := v.store.SaveSalt(backupData.Salt); err != nil {
	...
	if len(backupData.VaultMetadata) > 0 {
		if err := v.store.SaveMetadata(backupData.VaultMetadata); err != nil {
